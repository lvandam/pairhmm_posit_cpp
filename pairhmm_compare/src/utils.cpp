...
void writeBenchmark(PairHMMFloat<cpp_dec_float_100> &pairhmm_dec50, PairHMMFloat<float> &pairhmm_float,
                    PairHMMPosit &pairhmm_posit, std::string filename,
        ...
        outfile << "name,dE_f,dE_p,log(abs(dE_f)),log(abs(dE_p)),E,E_f,E_p,E_hw,da_F,da_P"
                << endl;
        for (int i = 0; i < dec_values.size(); i++) {
                cpp_dec_float_100 E, E_f, E_p, E_hw, dE_f, dE_p;
                cpp_dec_float_100 da_F, da_P; // decimal accuracies
                ...
                outfile << name << ",";
                outfile << setprecision(100) << fixed << dE_f << "," << dE_p << "," << flush;
                outfile << setprecision(100) << fixed << log10(abs(dE_f)) << "," << log10(abs(dE_p)) << "," << flush;
                outfile << setprecision(100) << fixed << E << "," << E_f << "," << E_p << "," << flush;
                outfile << setprecision(100) << fixed << da_F << "," << da_P << endl << flush;
        }
        outfile.close();
}
